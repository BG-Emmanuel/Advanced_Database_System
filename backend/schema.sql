...
-- PostgreSQL 14+ | Optimized for Cameroon Market
-- ================================================
...
-- Drop tables if rebuilding (reverse order for FK safety)
...
INSERT INTO attributes (attribute_name, attribute_type) VALUES
    ('Color','color'),('Size','size'),('Material','text'),('Brand','text'),
    ('Weight','number'),('Warranty','text'),('Storage','size'),('RAM','size'),
    ('Battery','text'),('Screen Size','number');
...
CREATE TRIGGER trg_vendor_updated    BEFORE UPDATE ON vendor_profiles FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE OR REPLACE FUNCTION update_product_rating()
RETURNS TRIGGER AS $$
BEGIN
    UPDATE products SET
        rating       = (SELECT COALESCE(AVG(rating),0) FROM product_reviews WHERE product_id = NEW.product_id),
...
-- ================================================
-- VISUAL SEARCH — Image embeddings for similarity search
-- ================================================

-- Enable pgvector extension (if available on your PostgreSQL host)
-- Supabase supports pgvector natively
CREATE EXTENSION IF NOT EXISTS vector;

-- Store image feature vectors for visual search
-- Each product image gets a 512-dimension embedding
CREATE TABLE IF NOT EXISTS product_image_embeddings (
    embedding_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    product_id UUID REFERENCES products(product_id) ON DELETE CASCADE,
    image_url VARCHAR(500) NOT NULL,
    embedding vector(512),           -- feature vector from vision model
    labels TEXT[],                   -- detected labels: ["phone","samsung","black"]
    colors TEXT[],                   -- dominant colors: ["black","silver"]
    category_hint VARCHAR(100),      -- AI-detected category
    confidence DECIMAL(4,3),         -- detection confidence 0-1
    created_at TIMESTAMPTZ DEFAULT NOW()
);

-- Index for fast vector similarity search
-- Using ivfflat for approximate nearest neighbor (ANN)
CREATE INDEX IF NOT EXISTS idx_embeddings_vector
    ON product_image_embeddings USING ivfflat (embedding vector_cosine_ops)
    WITH (lists = 100);

-- Index on labels for fallback text-based visual search
CREATE INDEX IF NOT EXISTS idx_embeddings_labels
    ON product_image_embeddings USING GIN (labels);

-- ================================================
-- VOICE SEARCH — Search query analytics
-- ================================================
CREATE TABLE IF NOT EXISTS search_analytics (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    user_id UUID REFERENCES users(user_id) ON DELETE SET NULL,
    query TEXT NOT NULL,
    search_type VARCHAR(20) DEFAULT 'text',  -- text | voice | visual
...
CREATE TABLE IF NOT EXISTS chats (
    ...
    vendor_id UUID REFERENCES vendor_profiles(vendor_id) ON DELETE CASCADE,
    vendor_user_id UUID REFERENCES users(user_id) ON DELETE CASCADE,
